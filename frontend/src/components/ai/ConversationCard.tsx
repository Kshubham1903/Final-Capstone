        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          <div
            className={`p-1.5 rounded-lg shrink-0 ${
              isActive
                ? "bg-purple-600/30 text-purple-theme border border-purple-500/30"
                : "bg-white/5 text-secondary-theme group-hover:text-main-theme"
            }`}
          >
          ...
          {isEditing ? (
            <form onSubmit={handleSaveRename} className="flex items-center gap-1 flex-1 min-w-0">
              <input
                ...
                autoFocus
                className="w-full glass-input text-main-theme text-xs px-2 py-1 rounded border border-purple-500 focus:outline-none"
              />
              <button
                type="submit"
                className="p-1 text-emerald-theme hover:text-emerald-300 cursor-pointer"
                title="Save"
              >
              ...
              <button
                ...
                className="p-1 text-secondary-theme hover:text-rose-400 cursor-pointer"
                title="Cancel"
              >
            ...
            <div className="min-w-0 flex-1">
              <h4 className={`text-xs font-semibold truncate ${isActive ? "text-main-theme" : "text-secondary-theme group-hover:text-main-theme"}`}>
                {conversation.title || "New Study Session"}
              </h4>
              <div className="flex items-center gap-2 mt-1">
                <span className="text-[10px] text-secondary-theme opacity-70">{formattedDate}</span>
                {conversation.learningMode && (
                  <span className="text-[9px] font-bold uppercase px-1.5 py-0.2 rounded bg-purple-500/10 text-purple-theme border border-purple-500/20">
                    {conversation.learningMode}
                  </span>
                )}
              </div>
